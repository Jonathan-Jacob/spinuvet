  ...
  class IngredientsController < AdminController
    ...
    def create
      ...
      Spina.locales.each do |locale|
        json_attributes.merge!("#{locale}_content" => {name: ingredient_params["#{locale}_name"], subtitle: ingredient_params["#{locale}_subtitle"], description: ingredient_params["#{locale}_description"], signed_blob_id: ingredient_params["#{locale}_signed_blob_id"], filename: ingredient_params["#{locale}_filename"], image_id: ingredient_params["#{locale}_image_id"]})
      end
      @ingredient = Spina::Ingredient.new(json_attributes: json_attributes, version_id: 1, version_counter: 1)
      if @ingredient.save
        Spina::IngredientDraft.create(json_attributes: @ingredient.json_attributes.dup, version_id: 1, ingredient_id: @ingredient.id)
    ...
    def update
      ...
      if @ingredient.save
        Spina::IngredientDraft.create(ingredient: @ingredient, json_attributes: @ingredient.json_attributes.dup, version_id: @ingredient.version_id) unless ingredient_params[:active_ingredient_draft].present?
